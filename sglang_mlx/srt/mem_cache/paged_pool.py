...
Layout (matches paged_attention_v1/v2 and reshape_and_cache Metal kernels):
  key_cache[layer]: (num_blocks, n_kv_heads, head_dim//x, block_size, x)  x=8 for fp16
...
# x-interleaving factor for the key cache (8 × fp16 = 16 bytes = one cache line)
_KEY_X = 8
...
class BlockAllocator:
    ...
    def free(self, block_ids: list[int]) -> None:
        """Return *block_ids* to the free list.
        ...
        """
        unique = list(dict.fromkeys(block_ids))  # preserve order, remove dups
        self._free.extend(unique)
...
class PagedMHAPool:
    ...
    where ``x = 8`` (vectorised loads for fp16).
    """
    ...
    def __init__(
        ...
        if head_dim % _KEY_X != 0:
            raise ValueError(
                f"head_dim must be divisible by {_KEY_X} for key-cache x-interleaving, "
        ...
        key_shape = (num_blocks, n_kv_heads, head_dim // _KEY_X, block_size, _KEY_X)
        val_shape = (num_blocks, n_kv_heads, head_dim, block_size)

        self.key_cache: list[mx.array] = [
            mx.zeros(key_shape, dtype=dtype) for _ in range(num_layers)
        ]
        self.val_cache: list[mx.array] = [
            mx.zeros(val_shape, dtype=dtype) for _ in range(num_layers)
        ]
...
class PagedKVCache:
    ...
    * ``compute_paged_attn(q, layer_idx, scale, max_seq_len)``  — compute decode
    ...
    as the old MHATokenPool.fetch — use ``store`` + ``compute_paged_attn`` (via a
    model patcher) to avoid the gather entirely.
    """
